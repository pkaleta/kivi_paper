...
Reduction depends on the type of node the algorithm encountered. As previously
said there are two cases. Supercombinator and built-ins reductions:

\subsubsection{Reducing supercombinators}
When supercombinator node is to be evaluated, first thing what G-machine does,
...
for formal parameters (as defined in Section 2.1.4 of \cite{joneslester00}).
...
+ (sqr 2)) * 3}. The graph built for this expression will be like in the
