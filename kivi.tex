...
This will result in value for expression \texttt{b} being caluclated twice.
Nevertheless a full laziness requires this value to be calculated at most once.
...
Each let(rec) should be floated until the next surrounding lambda
abstraction binds any of variables in let(rec)'s free variable set.


\subsection{Handling MFEs}
Another simple transformation will allow MFEs to be handled by algorithm
described for let(rec)s. In order to achieve it, every expression $e$ that
happens to be a maximally free subexpression of any enclosing lambda, should be
substituted with expression \texttt{let $v$ = $e$ in $v$}, where $v$ is a newly
introduced, unique variable. Let bindinges created in this way will become
available for let(rec) lifter described above.
