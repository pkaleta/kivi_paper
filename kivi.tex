...
\begin{lstlisting}[label=lst:letrec_dependency_example,caption={Example of
  \texttt{let(rec)} before dependency analysis.}]
letrec
    p       = 3;
    q       = 7;
    x       = p + q;
...
In order to fully understand the role of \textit{strongly connected components}
in analysing dependencies of a functional program I'll first introduce a few
definitions that will help with further deliberations.
\begin{definition}
...
From now on we'll be looking at each \texttt{letrec} expression as an directed
graph called \textit{dependency graph} defined as follows:
\begin{definition}
...
The dependency graph for our running example from
Listing~\ref{lst:letrec_dependency_example} will look like this:
