...
First chapter presents the parsing mechanisms used in Kivi. It is a description
...
Next chapter provides a description of translation processes that eventually
...
the parser has grown, I would strongly consider rewriting it to use parser combinator
...
Once we can accept tokens from lexer it is time to build a hierarchical
...
In this chapter I am going to present the process of transformation of a
high-level functional language into a simple intermediate form - \textit{lambda
calculus}. In the first part I will show \textit{Kivi}'s syntax, its
...
expression consisting of either constants or other supercombinators and does not
contain any free variables (see Definition~\ref{def:free_variable}) in its
body. Supercombinators might have arguments, and those which doesn not are called
...
arguments. It is the entry point for program execution. If there is no
...
\textit{Refutable} and \textit{irrefutable} ones. Lets look at this sample
...
bindings, it is time to know which of these groups depend on each other. In this
...
variables bound in current \texttt{letrec} binding. Otherwise it is simply the
...
\subsubsection{\texttt{Let(rec)} bindings}
In order to type-check \texttt{let} expressions, first type-checking of
bindings introduced by \texttt{let} takes place, obtaining an updated
environment, \texttt{typeInstanceEnv}. Body of \texttt{let} will by checked
using this fresh environment. The situation changes a bit in case of recursive
\texttt{let} expressions. Before analysing each definition it is necessary to
...
definition body as well as the body of a whole \texttt{letrec} in the end.
