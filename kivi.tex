...
Here's the point where the pattern matching algorithm starts its work. The goal
is to translate this form into one using \texttt{case} expressions. The code
responsible for this task has been placed in \textit{PatternMatching.hs} module
and will be used as a reference. The heart of algorithm is formed by
\texttt{match*} set of mutually recursive functions, specifically
\texttt{matchSc} is the one that performs pattern matching on supercombinators,
and \texttt{matchEquations} matches function definition equations.
\texttt{matchEquations} has the following type signature:
...
\begin{itemize}
  ...
  \item \texttt{dts}: A set of defined data types needed later in constructor
    rule (see ~\ref{sec:constructor_rule})
  \item \texttt{n}: The length of the list of argument variables
  \item \texttt{vs}: A list of argument variables
...
It works according to three rules depending on the type of currently considered
equation: the variable, constructor, mixture and empty rules. They are described in
next sections. Function \texttt{classifyEquation} returns this type given the
equation.
...
transform every expression corresponding to that pattern by one where every
...
function.
Our previous example - the \texttt{map} function eventually runs into situation
...
\begin{lstlisting}
    matchEquations [] [] Error
\end{lstlisting}
In this case it is equal to \texttt{Error} expression.
