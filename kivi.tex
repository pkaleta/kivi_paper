...
TODO: G-machine execution example?
...
implementation of efficient, non-strict evaluation strategy. Non-strictness in
this context means that function arguments are evaluated on demand. This is
also known as \textit{lazy evaluation}. Another feature of G-machine
implementation is that values should only be evaluated once, in order not to
repeat the same calculations over and over. The tree becomes a graph due to the
transformations that take place during graph reduction.
...
The process of reducing a graph consist of transforming a graph according to
the specified rules and eventually arriving to the state where it cannot be
reduced any more: the graph is in the so called \textit{weak head normal form}
(or WHNF for short). We assume that an expression is in the WHNF in several
cases. First of all it might be a number, character, or any other simple data
type. Secondly it might be a constructor application. Another situation where
there is nothing more to reduce if the expression is in ($f$ $e_1$ $e_2$
$\ldots$ $e_n$) where $f$ is lambda abstraction or built-in function and $n$ is
less than the expected number of arguments for $f$.

The process of evaluation can be described in four points:

\begin{itemize}
  \item Finding the next redex
  \item Check if the redex is in WHNF, if so stop, else continue
  \item Recucing the redex
  \item Updating the root of of the redex
\end{itemize}
